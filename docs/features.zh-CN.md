...
- **Agent 后台访问控制 (`allowBackgroundAccess`)**：每个控制器实例均具备独立的“允许 Agent 后台访问”开关（默认关闭）。未开启时仍可通过 jenkins_list_instances 列出（含标志）；其它 jenkins_* 工具需要显式开启。
...
- **流水线脚本编辑 (`at-jenkins:` 协议)**：
  - 通过可编辑草稿 URI 打开控制器存储的 CPS 流水线脚本 (`at-jenkins-draft://{instanceId}/Jenkinsfile?job={jobFullName}`)。
  - 原生 Groovy 语法高亮。
...
1. **`jenkins_list_instances`**：
   - 列出全部已配置控制器及标志（含 `allowBackgroundAccess`）；其它工具仍需开启后台访问。
   - 绝不返回 API Token、密码或敏感凭据。
2. **`jenkins_list_jobs`**：
   - 查询指定控制器的任务列表。
   - 支持可选的 `folderFullName` 参数以查询嵌套目录内的任务。
   - 返回任务名称、完整路径、类型 (`_class`)、颜色/状态及是否为文件夹标识。
3. **`jenkins_get_job`**：
   - 获取指定任务的详细元数据，包含参数定义与最近构建摘要。
...
6. **`jenkins_get_build`**：
   - 获取单次构建的详细信息（状态结果、耗时、时间戳及 Remote API 相关字段）。
7. **`jenkins_get_build_log`**：
   - 获取指定构建的控制台日志文本。
   - 默认截取末尾 64 KiB 日志（`DEFAULT_LOG_TAIL_BYTES`）。
   - 支持传入 `start` 字节偏移量进行增量分块读取，并返回分页元数据（`hasMore`、`totalBytes`、`startByte`、`endByte`、`truncated`）。
...
- 扩展激活时会自动安装/修复所有 AT 家族插件共享的单个 `AT Series` MCP 条目（Cursor、Kiro、Continue），安装 AT Jenkins 绝不会产生重复的独立 MCP 服务条目。
