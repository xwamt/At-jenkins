...
1. `jenkins_list_instances`: Lists all configured Jenkins instances with flags (including `allowBackgroundAccess`). Never returns tokens or passwords. Other jenkins_* tools require `allowBackgroundAccess: true` on the target instance.
2. `jenkins_list_jobs`: Lists top-level jobs or jobs within a folder (optional `folderFullName` param). Returns job name, full name, class, color/status, and whether it is a folder.
...
6. `jenkins_get_build`: Gets build details including result status, duration, timestamp, and related Remote API fields.
7. `jenkins_get_build_log`: Gets build console log text with tail truncation support (defaults to 64 KiB tail; accepts `start` byte offset; returns `hasMore` / `endByte` / `truncated`).

## Core workflow

1. `jenkins_list_instances`. If none have `allowBackgroundAccess: true`, ask the user to enable **"Allow Agent background access"** on the target controller.
2. Discovery: `jenkins_list_jobs` (optionally specifying `folderFullName`) to find target jobs.
...
5. For large logs: use `start` parameter or inspect `hasMore` / `endByte` / `totalBytes` to stream chunks.
